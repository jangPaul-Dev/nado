...
public class ApplicationContext {
	...
	private void prepareAnnotationObjects() throws Exception {
		/*
		 * Reflections(패키지) : 해당 패키지 하위를 모두 찾는다 Reflections("spms") : spms 하위를 모두 찾는다
		 * Reflections("spms.controls") : spms.controls 하위를 모두 찾는다 Reflections("") :
		 * classpath 하위를 모두 찾는다
		 */
...
// 메서드 중에 Setter를 찾아라
			/*
			 * Dao객체는 set으로 시작하는 메서드가 없으므로 주입 대상이 되지 않는다
			 */
			/* pageController객체만 Setter가 있다 */
			if (m.getName().startsWith("set")) {
				/*
				 * 현재 objTable로부터 첫번째 매개변수에 해당하는 클래스 객체를 찾아라 => 즉 Dao객체를 찾아라
				 */
